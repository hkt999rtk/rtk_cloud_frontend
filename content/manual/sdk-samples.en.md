...
| Device reference clients | Linux simulator, PRO2 camera device demo | Validate device-side command handling, sample MQTT payload behavior, status/log/event reporting, snapshot upload, and the WebRTC answerer boundary. |
...
4. Connect the PRO2 camera device demo when hardware is available to validate device-bound token flow, owner transport, snapshot upload, camera status/log/event reporting, and WebRTC answerer behavior.
