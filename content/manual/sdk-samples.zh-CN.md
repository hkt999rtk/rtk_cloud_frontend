...
![Realtek Connect+ 范例生态系](/static/assets/connectplus-sample-ecosystem.png)
...
| 装置参考客户端 | Linux 模拟器、PRO2 相机装置范例 | 验证装置端 command handling、sample MQTT payload 行为、状态/log/event 回报、snapshot upload 与 WebRTC answerer 边界。 |
...
4. 有 PRO2 硬体时接上 PRO2 camera device demo，验证 device-bound token、owner transport、snapshot upload、相机 status/log/event 回报与 WebRTC answerer 行为。
