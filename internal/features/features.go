	...
	return []Feature{
		{
			...
			Description:  "Provision presents Realtek Connect+ onboarding as a layered path from registry entry to activated cloud device. The current foundation is the account registry, cross-service provisioning command flow, WebRTC Video over TURN activation boundary, service-scoped provisioning credentials, and transport readiness contract; local Wi-Fi/BLE setup, QR/SoftAP UX, ownership transfer, factory reset policy, and aggregate product readiness are not presented as generally available implementation until those owner repositories land them.",
			ImagePath:    "/static/assets/connectplus-platform-surfaces-corporate-v2.jpg",
			ImageAlt:     "Enterprise platform surface showing device registry, onboarding status, dashboard panels, and a mobile companion view.",
			...
			Highlights:   []string{"Contract-backed cloud registry, activation, service credential, and transport readiness boundaries", "Integration-ready claim material concepts for QR, serial, activation code, and future factory identity", "Roadmap treatment for local Wi-Fi/BLE setup, SoftAP UX, transfer/reset policy, and aggregate readiness"},
			...
			Outcomes:     []string{"Preserve the product onboarding vision without overclaiming implementation status", "Give firmware, app, SDK, account, and WebRTC Video over TURN teams one shared availability vocabulary", "Make evaluation conversations explicit about what is available now, integration-ready, or roadmap"},
			Sections: []FeatureSection{
				{
			...
			Table: FeatureTable{
				...
				Rows: []FeatureTableRow{
					{Cells: []string{"Cloud registry and activation foundation", "Available foundation", "Account registry, cross-service provisioning commands and events, WebRTC Video over TURN activation, scoped credentials, and transport readiness define the current cloud-side path."}},
		...
		},
		{
			...
			ImagePath:    "/static/assets/connectplus-operations-console-corporate-v2.jpg",
			ImageAlt:     "Enterprise operations console showing firmware rollout, device registry, fleet health charts, and alert panels.",
		...
		},
		{
			...
			ImagePath:    "/static/assets/connectplus-operations-console-corporate-v2.jpg",
			ImageAlt:     "Enterprise operations console showing device registry, fleet health charts, rollout status, and operational alerts.",
		...
		},
		{
			...
			ImagePath:    "/static/assets/connectplus-platform-surfaces-corporate-v2.jpg",
			ImageAlt:     "Enterprise platform surfaces showing mobile app context, connected device panels, and operations dashboard views.",
		...
		},
		{
			...
			Capabilities: []string{"Forgot-password, change-password, and session-management controls", "Account deletion and retention workflows that hand off cleanly to support and compliance teams", "User profile, consent, and device-ownership state that stays separate from this marketing website", "Product roles and permissions remain account-side authorization contract content until the platform contract is finalized"},
		...
		},
		{
			...
			ImagePath:    "/static/assets/connectplus-sample-ecosystem-corporate-v2.jpg",
			ImageAlt:     "Realtek Connect+ sample ecosystem diagram with generic app clients, simulator, reference device, and central platform hub.",
		...
		},
		{
			...
			ImagePath:    "/static/assets/connectplus-operations-console-corporate-v2.jpg",
			ImageAlt:     "Enterprise operations console showing fleet health charts, firmware distribution, telemetry summaries, and alert panels.",
		...
		},
		{
			...
			ImagePath:    "/static/assets/connectplus-architecture-diagram-corporate-v2.jpg",
			ImageAlt:     "Enterprise architecture diagram showing secure device connections, cloud services, APIs, MQTT, webhooks, and ecosystem endpoints.",
			...
			Table: FeatureTable{
				...
				Rows: []FeatureTableRow{
					...
					{Cells: []string{"REST APIs", "Expose authenticated HTTPS endpoints for device, user, and operations workflows in external systems.", "API contracts, authorization policy, and lifecycle governance stay under the product team's platform boundary; this website does not publish or enforce product ACLs.", "Partner portals, support tooling, ERP/CRM integration, and enterprise admin workflows."}},
		...
		},
		{
			...
			ImagePath:   "/static/assets/connectplus-architecture-diagram-corporate-v2.jpg",
			ImageAlt:    "Enterprise architecture diagram showing device identity, PKI trust boundaries, cloud services, and integration endpoints.",
			...
			Table: FeatureTable{
				...
				Rows: []FeatureTableRow{
					...
					{Cells: []string{"Mutual TLS enforcement", "Client certificate requirement on MQTT broker and HTTPS device API endpoints.", "Certificate validation policy and device endpoint rules are configurable per fleet or deployment tier; human roles remain account-side authorization contract scope."}},
