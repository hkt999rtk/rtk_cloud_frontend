...
func zhTWText() map[string]string {
	return map[string]string{
		...
		"home.deploy.public.body":    "透過共用評估環境讓韌體、行動、雲端與產品團隊在商用部署前對齊。",
		...
		"privacy.data.body":          "聯絡表單可能收集姓名、公司、Email、關注服務與選填訊息。Analytics event rows 僅限 page_view、click_cta、scroll 與 engaged events、canonical page 或 CTA keys、必要時的粗略 scroll percent 或 duration、referrer origin only，以及短期 ephemeral session id。Analytics 不收集姓名、Email、公司、表單訊息、raw IP addresses、full referrer URLs 或 account identifiers。",
		"privacy.use.title":          "資料使用方式",
		"privacy.use.body":           "我們使用提交資料來回覆詢問、規劃產品討論、了解 Realtek Connect+ 服務需求，並保護網站免於 spam 或濫用。當 ANALYTICS_ENABLED=true 時，第一方 SQLite analytics 僅用於了解此網站的粗略流量來源、頁面互動、CTA conversion 與內容成效。",
		"privacy.retention.title":    "保存期限",
		"privacy.retention.body":     "網站 leads 預期最多保存 24 個月，除非仍有進行中的商務討論或必要營運紀錄需要更長期間。Raw analytics event rows 預設透過 ANALYTICS_RETENTION_DAYS 保存 90 天。",
...
func zhTWFeatures() map[string]localizedFeature {
	return map[string]localizedFeature{
		...
		"private-cloud": {
			Title:        "私有雲",
			Kicker:       "部署到任何雲端或地端環境，無 Serverless 綁定。",
			Summary:      "Realtek Connect+ 私有部署以 Container 或 VM 工作負載運行，可落在 GCP、Azure、AWS 或自有資料中心，無特定雲端供應商依賴。",
			Description:  "私有雲說明 Realtek Connect+ 的兩種部署層級與基礎架構模型。平台以標準 Container 或 VM 工作負載運行，讓客戶自行選擇雲端供應商或地端基礎架構，明確區隔於將私有部署綁定單一雲端帳號的 Serverless-native IoT 平台。",
			ImageAlt:     "顯示 Container 與 VM 工作負載跨多個雲端供應商與地端資料中心運行的私有雲架構。",
			Highlights:   []string{"支援 GCP、Azure、AWS 或地端資料中心的 Container/VM 部署，無雲端綁定", "免費評估最高可申請 200 台裝置，高於常見評估上限", "商用層級採一次性授權加年度維護，無最低規模門檻"},
			Capabilities: []string{"在主要雲端供應商或地端資料中心部署 Container 或 VM 工作負載", "反向代理 TLS、網路政策與品牌服務端點", "從評估到生產的發布提升與維護窗口"},
			Outcomes:     []string{"避免 IoT 基礎架構的雲端供應商綁定", "明確資料所有權與所在地邊界", "依客戶自有基礎架構建立從 pilot 到 production 的可信路徑"},
		},
		"integrations": {
			Title:        "生態系整合",
