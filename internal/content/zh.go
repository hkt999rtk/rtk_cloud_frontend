...
func zhTWText() map[string]string {
	return map[string]string{
		...
		"privacy.data.body":                             "聯絡表單可能收集姓名、公司、Email、關注服務與選填訊息。Analytics event rows 僅限 page_view、click_cta、scroll milestone 與 engaged-duration events、canonical page 或 CTA keys、必要時的粗略 scroll percent 或 engagement duration、referrer origin only，以及短期 ephemeral session id。Analytics 不收集姓名、Email、公司、表單訊息、raw IP addresses、full referrer URLs 或 account identifiers，且在 MVP 中不使用 cookies、localStorage 或其他持久化瀏覽器儲存。",
		"privacy.use.title":                             "資料使用方式",
		"privacy.use.body":                              "我們使用提交資料來回覆詢問、規劃產品討論、了解 Realtek Connect+ 服務需求，並保護網站免於 spam 或濫用。當 ANALYTICS_ENABLED=true 時，第一方 analytics 僅用於了解網站效能、粗略流量來源、頁面互動、CTA conversion、內容成效、產品行銷改善與服務品質觀察。",
		"privacy.retention.title":                       "保存期限",
		"privacy.retention.body":                        "網站 leads 預期最多保存 24 個月，除非仍有進行中的商務討論或必要營運紀錄需要更長期間。Raw analytics event rows 預設透過 `ANALYTICS_RETENTION_DAYS` 保存 90 天，並與聯絡 lead 記錄分開保存。若之後實作 aggregate metrics，則可保存更久，但任何更長期限都必須另外限制並獨立說明。",
		"privacy.rights.title":                          "查詢、更正或刪除請求",
		"privacy.rights.body":                           "如需查詢、更正或刪除已提交的詢問資料，或詢問第一方 analytics，請聯絡 privacy@example.com。正式公開前需將此 placeholder 信箱替換為正式隱私聯絡窗口。由於 analytics 只使用 ephemeral session id，且沒有 account identifier，本站可能無法可靠地將單筆 raw analytics row 對應到特定個人。",
		...
		"privacy.legal.body":                            "此聲明是網站 prototype 的 GDPR-lite 準備說明，不是完整法律合規套件，且依適用法律 analytics 記錄仍可能屬於個人資料。公開上線前應完成審閱。公開啟用 analytics 會持續被 issue #52 與 #54 的披露更新所阻擋，直到兩者都到位。",
	}
}
