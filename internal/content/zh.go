...
func zhTWText() map[string]string {
	return map[string]string{
		...
		"privacy.intro":                                 "第一版網站只收集回覆 Realtek Connect+ 商務詢問與早期評估請求所需的資訊。當 ANALYTICS_ENABLED=true 時，也會使用第一方 SQLite analytics 做基本網站量測。",
...
func zhTWFeatures() map[string]localizedFeature {
	return map[string]localizedFeature{
		...
		"security": {
			...
			ImageAlt:     "顯示 root CA、issuing CA、裝置憑證與 mTLS 雲端連線的 PKI 階層圖。",
			SourceLabel:  "平台安全合約",
