...
func zhTWText() map[string]string {
	return map[string]string{
		...
		"home.deploy.title":                         "從評估開始，部署到任何雲端或地端機房。",
		"home.deploy.public":                        "公有評估",
		"home.deploy.public.title":                  "在投入私有部署前驗證產品適配性。",
		"home.deploy.public.body":                   "免費評估方案——預設 5 台裝置，可申請放寬到 200 台。沒有時間限制。在進入商用部署規劃前，用評估環境讓韌體、行動、雲端與產品團隊先對齊。",
		...
		"contact.interest.option.commercial-deployment": "商業部署（私有雲、報價、簽約）",
...
func zhTWFeatures() map[string]localizedFeature {
	return map[string]localizedFeature{
		"provision": {
			...
			Description:  "Provision 將 Realtek Connect+ 導入描述為從 registry 到雲端啟用裝置的分層流程。現階段可對外說明的是帳號 registry、跨服務 provision command、WebRTC Video over TURN 啟用邊界、scoped token 與 transport readiness 合約；本地 Wi-Fi/BLE 設定、QR/SoftAP UX、所有權轉移、factory reset 政策與彙整產品就緒狀態，在負責 repo 完成前不描述為全面可用實作。",
		...
		"private-cloud": {
			...
			ImageAlt:     "顯示裝置身分、私有雲服務、資料儲存與整合端點的企業架構圖。",
		...
		"security": {
			...
			Description:  "安全與 PKI 說明 Realtek Connect+ 如何以建立在 X.509 憑證上的公開金鑰基礎架構來驗證裝置身分、保護雲端通訊，並在不自訂通訊協定的前提下支援大規模撤銷。每台裝置在佈建時都會取得由平台 CA 階層簽發的唯一憑證。雲端連線一律透過該憑證進行 mutual TLS 驗證，讓平台可以確認硬體身分、套用政策，並以標準工具在大規模裝置群中輪替或撤銷憑證。",
			...
			Capabilities: []string{"維持兩層 CA 階層，讓 root CA 保持離線，並由 issuing CA 在佈建流程中按需簽發裝置憑證", "將每張裝置憑證綁定序號、MAC 位址與型號，讓雲端無需共享密鑰即可驗證硬體身分", "在 MQTT 與 HTTPS 端點強制 mutual TLS，讓未通過驗證的裝置無法存取平台 API 或訊息代理", "依憑證到期排程或安全事件支援更新與輪替，而不必完整重新佈建裝置", "發布 CRL 端點並執行 OCSP responder，讓依賴方可即時查驗憑證狀態", "可透過裝置群管理主控台撤銷單一裝置憑證，或批次撤銷遭入侵的製造批次"},
