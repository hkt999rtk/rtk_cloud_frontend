...
func enText() map[string]string {
	return map[string]string{
		...
		"privacy.use.body":                              "We use submitted information to respond to inquiries, plan product discussions, understand interest in Realtek Connect+ services, and protect the website from spam or abuse. First-party analytics is used only when `ANALYTICS_ENABLED=true` to understand site performance, coarse traffic source, page engagement, CTA conversion, content effectiveness, product-marketing improvement, and service quality observation for this website.",
		"privacy.retention.title":                       "Retention",
		"privacy.retention.body":                        "Website leads are intended to be retained for up to 24 months unless a longer period is needed for an active business discussion or required operational record. Raw analytics event rows are retained for 90 days by default through `ANALYTICS_RETENTION_DAYS` and are kept separate from contact lead records. Aggregate metrics may be retained longer if they are implemented later, but any longer period must be capped and documented separately.",
