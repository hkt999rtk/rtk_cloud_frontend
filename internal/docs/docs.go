...
func All() []Section {
	return []Section{
		...
		},
		{
			...
			Table: Table{
				...
				Rows: []TableRow{
					...
					{Cells: []string{"Linux device simulator", "Device reference", "Light, AC, meter, state, log, report, and snapshot-metadata workflows; no camera frames or WebRTC signaling."}},
					{Cells: []string{"PRO2 camera device demo", "Device firmware reference", "Host smoke validates adapter wiring and signaling lifecycle; physical hardware is required for camera/audio media validation."}},
				},
			},
		},
		{
		...
		},
		{
			...
			Title:        "Deployment",
			Icon:         "cloud-lock",
			Kicker:       "Document the production runtime profile behind evaluation and commercial deployments.",
			Summary:      "Production deployment profile, persistent SQLite storage, reverse proxy TLS, health checks, backup/restore, and rollback notes.",
			Description:  "Deployment explains how to run the Go site in a production profile without moving TLS termination into the app itself. It covers the container or VM runtime, the persistent SQLite volume used for lead and analytics data, the health-check endpoint, cache expectations at the edge, and the backup/restore path for production operations.",
			Highlights:   []string{"Container or VM deployment profile with baked templates and static assets", "Persistent SQLite volume for lead submissions and analytics events", "TLS termination, cache headers, and probes handled by the reverse proxy or hosting edge", "Backup, restore, and rollback checkpoints for production releases"},
			Deliverables: []string{"Production deployment checklist", "SQLite backup and restore runbook", "Rollback and verification steps for operators"},
			Audience:     []string{"Platform teams planning hosted environments", "Operations teams handling production recovery", "Enterprise buyers evaluating data and recovery boundaries"},
		},
		{
			Slug:         "release-notes",
			Title:        "Release Notes",
